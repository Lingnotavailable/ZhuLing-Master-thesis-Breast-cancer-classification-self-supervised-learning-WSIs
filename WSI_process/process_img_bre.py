...
image_folder = r'data\BreastPathQ\breastpathq\datasets\validation'
output_dir = r'data\IMG-SPLI-BRE\val'
...
def crop_image(image_folder_path,image_name,image_folder):
    ...
    for x in range(x_tiles):
        for y in range(y_tiles):
            ...
            tile_path = os.path.join(image_folder,f'breast_{image_name[:-4]}_{x}_{y}.png')  # 保存为PNG格式
            tile.save(tile_path)
...
for index,image_name in enumerate(image_list):
    crop_image(image_folder,image_name,os.path.join(output_dir))
    print(f'{index}/{image_folder}')