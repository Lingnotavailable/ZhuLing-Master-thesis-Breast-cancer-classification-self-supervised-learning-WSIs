...
def parse_args():
    ...
    parser.add_argument('--config',default='work_dirs/vit-base-p16_64xb64_in1k_wsi_Dataset100/vit-base-p16_64xb64_in1k_wsi_Dataset100.py', help='tsne config file path')
    parser.add_argument('--checkpoint', default="work_dirs/vit-base-p16_64xb64_in1k_wsi_Dataset100/epoch_300.pth", help='checkpoint file')
    parser.add_argument('--work-dir', default='work_dirs',help='the dir to save logs and models')
    parser.add_argument(
        '--test-cfg',
        default='work_dirs/resnet50_b96_in1k_wsi_Dataset100/resnet50_b96_in1k_wsi_Dataset100.py',
...
def main():
    ...
    classes_dict = {'Benign':0,'InSitu':1,'Invasive':2,'Normal':3}
    for i in range(len(dataset)):
        data_information = dataset.get_data_info(i)
        gt_label =classes_dict[data_information['img_path'].split('/')[-2]]
    ...
    for data in progress.track(dataloader, description='Calculating...'):
        with torch.no_grad():
            ...
            batch_labels = torch.Tensor([classes_dict[i.img_path.split('/')[-2]] for i in batch_data_samples])
