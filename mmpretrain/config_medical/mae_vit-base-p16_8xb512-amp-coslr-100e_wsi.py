...
default_hooks = dict(
    checkpoint=dict(interval=3, max_keep_ckpts=30, type='CheckpointHook'),
...
log_level = 'INFO'
model = dict(
    backbone=dict(arch='b', mask_ratio=0.75, patch_size=16, type='MAEViT'),
    head=dict(
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        betas=(
            0.9,
            0.95,
        ), lr=0.00005, type='AdamW', weight_decay=0.05),
    paramwise_cfg=dict(
        custom_keys=dict(
...
work_dir = './work_dirs/mae_vit-base-p16_8xb512-amp-coslr-300e_in1k'
